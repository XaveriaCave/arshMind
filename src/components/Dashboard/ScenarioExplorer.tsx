...
import { AlertTriangle, Zap, TrendingUp, Shield, Rocket, Globe, DollarSign, Briefcase, Sparkles, Loader2 } from "lucide-react";
...
export default function ScenarioExplorer({
   ...
   // NEW: separate handler that also saves feedback text
   ...
   const handleReplan = async (scenario: Scenario) => {
      ...
      // Capture feedback text now before it gets cleared
      ...
      try {
         const res = await fetch(`${API_BASE}/api/replan-path`, {
            ...
            body: JSON.stringify({ profile, scenario, feedback: feedbackText }),
         });

         if (!res.ok) throw new Error(`Re-calibration failed with status: ${res.status}`);
         const data = await res.json();
         if (data.error) throw new Error(data.error);

         // Build new scenario object with history preserved
         ...
         // Call onReplan — this saves BOTH the feedback text AND the new scenario to Firestore
         await onReplan(scenario, feedbackText, recalibrated);
   ...
   const executeRestore = async () => {
      ...
      try {
         ...
         if (!currentScenario) { setPendingRestore(null); return; }

         const updatedScenarios = scenarios.map(item =>
            item.id === expandedId ? { ...pendingRestore } : item
         );
      ...
      <div className="space-y-16">
         <header className="border-l-2 border-emerald-500 pl-8 pb-4">
            ...
            <p className="text-slate-500 text-[10px] font-mono tracking-[0.3em] uppercase">
               Simulating 6 AI-modeled future trajectories for your profile
            </p>
         </header>
            ...
            {scenarios.map((s) => (
               <motion.div
                  ...
                  className={cn(
                     ...
                     expandedId === s.id
                        ? "border-emerald-500/50 bg-emerald-500/5 shadow-[0_10px_40px_rgba(16,185,129,0.1)]"
               ...
               >
                  {activeScenarioId === s.id && (
                     <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500 opacity-20"
                  ...
                  <h3 className="text-xl font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors uppercase tracking-tight">
                     {s.title}
                  </h3>
         ...
         <AnimatePresence mode="wait">
            {expandedId && (() => {
               ...
               return (
                  ...
                  >
                     <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500 opacity-[0.03] pointer-events-none"
                        style={{ clipPath: "polygon(100% 0, 100% 100%, 0 0)" }} />
                     ...
                     <div className="space-y-12">
                        ...
                        <div className="flex flex-col lg:flex-row justify-between gap-10">
                           <div className="flex-1">
                              ...
                              <p className="text-slate-400 text-[14px] leading-relaxed max-w-3xl font-medium border-l border-emerald-500/20 pl-6">
                                 {s.description}
                              </p>
                           </div>
                           ...
                           {!profile?.isPro ? (
                              <div className="p-6 border-geom border-amber-500/20 bg-amber-500/[0.02] flex flex-col md:flex-row md:items-center justify-between gap-6 text-left">
                                 <div className="space-y-1">
                                    ...
                                    <p className="text-[11px] text-slate-500 max-w-xl font-medium leading-relaxed font-sans">
                                       Upgrade to Pro to customize this path trajectory dynamically. Provide specific limitations,
                                       problems, or constraint boundaries and let ArshMind recalculate your 5-year yields on the fly.
                                    </p>
                                 </div>
                                 <button
                              ...
                              <div className="p-6 border-geom border-white/10 bg-black/40 space-y-4">
                                 <p className="text-[11px] text-slate-400 font-medium leading-relaxed text-left font-sans">
                                    Inject custom parameters (constraints, bottlenecks, or alternatives) for this path.
                                    ArshMind will completely recalibrate descriptors, financial yields, and project milestones
                           ...
                           <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
                              ...
                              >
                                 ...
                                 <p className="text-[11px] text-slate-300 leading-relaxed font-mono uppercase">
                                    WARNING: YOU ARE ABOUT TO OVERWRITE THE ACTIVE TIMELINE WITH RESTORE POINT
                                    "v{s.history ? s.history.length - s.history.indexOf(pendingRestore) : "X"}" ({pendingRestore.title}).
                                 </p>
                                 <p className="text-[11px] text-red-500 font-mono italic leading-relaxed uppercase border-l-2 border-red-500/40 pl-4 py-1">
                        // CORE PROTOCOL WARNING: These alternate timeline changes will be IRREVERSIBLE if after
                                    switching back, you ask or prompt a new dynamic limitation or constraints vector on ArshMind.
                                 </p>
                        ...
                        <div className="pt-10 border-t border-white/5 flex justify-end">
                           ...
                           >
                              {activeScenarioId === s.id ? "Status: Active" : "Set Active Vector"}
                           </button>
                        </div>
                     </div>
                  </motion.div>
...
function StatBox({ label, value }: { label: string; value: string }) {
   return (
      <div className="min-w-[140px] p-5 bg-black border-geom border-white/10 flex flex-col items-center justify-center text-center">
